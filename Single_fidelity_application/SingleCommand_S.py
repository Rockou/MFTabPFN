"""
Single-command script to reproduce single-fidelity prediction and UQ
This will produce Figs. 2, 3, 4, 5, and 6
...
def run_script(script_name: str):
    ...
    if not script_path.exists():
        ...
        sys.exit(1)
    if script_name in ["HDR_S_Plot.py", "HDR_S_row_Plot.py", "HDR_S_Time_Plot.py", "HDR_S_Wilcoxon_Plot.py", "HDR_VaryDataSize_S_Plot.py", "UQ_S_Plot.py"]:
    ...
    print(f"{'='*80}")
    try:
        subprocess.run([sys.executable, run_path], cwd=".", check=True)
...
def main():
    # Determine repository root (one level up from Single_fidelity_application/)
    ...
    scripts_to_run = [
        "HDR_Performance_S.py",      # High-dimensional performance processing
        "P_CTR23_S.py",              # OpenML-CTR23 benchmark processing
        "HDR_VaryDataSize_S.py",     # Performance under varying data size
        "UQ_Prediction_S.py",        # UQ Performance
        "HDR_S_Plot.py",             # Normalized performance metrics plots
        "HDR_S_row_Plot.py",         # Row performance metrics plots
        "HDR_S_Time_Plot.py",        # Computational time violin plots
        "HDR_S_Wilcoxon_Plot.py",    # Statistical significance heatmap
        "HDR_VaryDataSize_S_Plot.py",# Performance metrics plots under varying data size
        "UQ_S_Plot.py",              # UQ performance plots
    ]

    print("Starting full reproduction of single-fidelity prediction results...\n")
    ...
    print("="*80)
    print("All single-fidelity prediction problems finished!")
    print("="*80)
