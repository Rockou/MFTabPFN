import os
# Disable TorchDynamo (to avoid potential compilation issues in some environments)
os.environ["TORCHDYNAMO_DISABLE"] = "1"
# Allow duplicate lib usage (common fix for some MKL/OpenBLAS conflicts on certain systems)
...
import gc
# Enforce GPU usage — script will exit if no GPU is found
...
def UtoX(u, mu, sigma, DisType):
    """Transform standard normal samples u to target distribution X."""
    x = np.zeros_like(u)
...
def XtoU(x, mu, sigma, DisType):
    """Reverse transformation: from target distribution X back to standard normal U."""
    u = np.zeros_like(x)
    for i in range(len(mu)):
        if DisType[i] == 'normal':
...
# Experiment settings
n_performances = 5                          # number of different function cases (0~4)
n_simulations = n_repeats * n_splits        # total number of train/test splits
Results_simulation_list = [[None for _ in range(n_simulations)] for _ in range(n_performances)]
Results_simulation_prediction = [[None for _ in range(n_simulations)] for _ in range(n_performances)]
random_seeds_train = np.array([111, 222, 333])

n_hyperparameter_configs1 = 100
num_gpus1 = 1
time_limit1 = 3600                          # seconds
# Main experiment loop over different function cases
for j in range(0, n_performances):
    Case = j
    mu, sigma, DisType = dis(Case)
    nx = mu.shape[0]                        # dimensionality of the problem
    N = 3 * nx                              # total number of samples
    np.random.seed(123)

    # Generate standard normal samples and transform to target distribution
    ...
    # Prepare lists of train/test splits
    XX_train_yuan_list = []
    XX_total_yuan_list = []
    YY_train_yuan_list = []
    YY_total_yuan_list = []
    ...
    # Run experiments for each fold/split
    for k in range(n_simulations):
        ...
        # Preprocess features (fit on train, transform both train & test)
        ...
        # Model hyperparameters (mostly fixed across experiments)
        input_TabPFN = np.min([np.max([20, nx]), 500])
        n_layer = 3
        hidden_channels = np.max([128, 2 * nx])
        activate_function = 'tanh'          # options: 'tanh', 'sigmoid', 'relu'
        lr = 0.001
        weight_decay = 1e-3
        epochs = 100
        bili = 1.0                          # train ratio (here 100% of split)
        task_type = "regressor"
        batch = 256
        ...
        Results_simulation_list[j][k] = pd.DataFrame(results_list)
        Results_simulation_prediction[j][k] = pd.DataFrame(results_prediction)
        with open('Results_simulation_list.pkl', 'wb') as f:
            pickle.dump(Results_simulation_list, f)
        with open('Results_simulation_prediction.pkl', 'wb') as f:
            pickle.dump(Results_simulation_prediction, f)
        del predictor
