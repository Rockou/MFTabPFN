...
n_simulations = 3
Results_ONERAM6_list_ANMFGPR = [None for _ in range(n_simulations)]
Results_ONERAM6_prediction_ANMFGPR = [None for _ in range(n_simulations)]
...
for j in range(0, n_simulations):
    ...
    for i in range(U):
        ...
        results_list.append(
            {'Model': 'NonlinearMFGP', 'Time_Train': train_time, 'Time_Pred': pred_time, 'Index_fidelity': index_fidelity,
             'Index_max': index_max, 'N_low': N_low, 'N_high': N_high})
        results_prediction.append({'Model': 'NonlinearMFGP', 'Performance': Y_test_prediction_NonlinearMFGP, 'Performance_low': Y_test_prediction_NonlinearMFGP_low,
    ...
    Results_ONERAM6_list_ANMFGPR[j] = pd.DataFrame(results_list)
    Results_ONERAM6_prediction_ANMFGPR[j] = pd.DataFrame(results_prediction)
    with open('Results_ONERAM6_list_ANMFGPR.pkl', 'wb') as f:
        pickle.dump(Results_ONERAM6_list_ANMFGPR, f)
    with open('Results_ONERAM6_prediction_ANMFGPR.pkl', 'wb') as f:
        pickle.dump(Results_ONERAM6_prediction_ANMFGPR, f)
