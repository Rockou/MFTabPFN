...
n_stop = len(stop0)
n_simulations = 3
Results_ONERAM6_list_FNO = [[None for _ in range(n_simulations)] for _ in range(n_stop)]
Results_ONERAM6_prediction_FNO = [[None for _ in range(n_simulations)] for _ in range(n_stop)]
...
for j in range(0, n_simulations):
    for k in range(0, n_stop):
        ...
        results_list = []
        results_list.append(
            {'Model': 'FNO', 'Time_Train': train_time, 'Time_Pred': pred_time, 'N_low': N_low, 'N_high': N_high, 'Stop': stop0[k]})
        Y_test_prediction_FNO = Y_test_prediction.copy()
        Y_test_prediction_FNO_low = Y_test_prediction_low.copy()
        results_prediction = []
        results_prediction.append({'Model': 'FNO', 'Y_test_prediction': Y_test_prediction_FNO,
                                   'Y_test_prediction_low': Y_test_prediction_FNO_low})

        Results_ONERAM6_list_FNO[j][k] = pd.DataFrame(results_list)
        Results_ONERAM6_prediction_FNO[j][k] = pd.DataFrame(results_prediction)
        with open('Results_ONERAM6_list_FNO.pkl', 'wb') as f:
            pickle.dump(Results_ONERAM6_list_FNO, f)
        with open('Results_ONERAM6_prediction_FNO.pkl', 'wb') as f:
            pickle.dump(Results_ONERAM6_prediction_FNO, f)
