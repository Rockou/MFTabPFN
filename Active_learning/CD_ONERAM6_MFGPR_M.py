...
n_stop = len(stop0)
n_simulations = 3
Results_ONERAM6_list_MFGPR = [[None for _ in range(n_simulations)] for _ in range(n_stop)]
Results_ONERAM6_prediction_MFGPR = [[None for _ in range(n_simulations)] for _ in range(n_stop)]
...
for j in range(0, n_simulations):
    for k in range(0, n_stop):
        ...
        results_list = []
        results_list.append(
            {'Model': 'LinearMFGP', 'Time_Train': train_time, 'Time_Pred': pred_time, 'N_low': N_low, 'N_high': N_high, 'Stop': stop0[k]})
        results_prediction = []
        results_prediction.append({'Model': 'LinearMFGP', 'Y_test_prediction': Y_test_prediction_LinearMFGP,
                                   'Y_test_prediction_low': Y_test_prediction_LinearMFGP_low})

        Results_ONERAM6_list_MFGPR[j][k] = pd.DataFrame(results_list)
        Results_ONERAM6_prediction_MFGPR[j][k] = pd.DataFrame(results_prediction)
        with open('Results_ONERAM6_list_MFGPR.pkl', 'wb') as f:
            pickle.dump(Results_ONERAM6_list_MFGPR, f)
        with open('Results_ONERAM6_prediction_MFGPR.pkl', 'wb') as f:
            pickle.dump(Results_ONERAM6_prediction_MFGPR, f)
