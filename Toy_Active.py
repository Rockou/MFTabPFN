...
for i in range(U):
    ...
    Prediction_sigma_initial_mean = np.mean(model_MLP.afa.detach().cpu().numpy() * Prediction_sigma_initial)
    ...
    if i < 5:
        Prediction_sigma_stack = np.stack((min_distances_low * model_MLP.afa.detach().cpu().numpy() * Prediction_sigma_initial * Prediction_sigma_initial_mean / Cost_low, min_distances_high * Prediction_sigma * Prediction_sigma_mean / Cost_high), axis=1)
    else:
        Prediction_sigma_stack = np.stack((model_MLP.afa.detach().cpu().numpy() * Prediction_sigma_initial * Prediction_sigma_initial_mean / Cost_low, Prediction_sigma * Prediction_sigma_mean / Cost_high),axis=1)
...
print("Active MFTabPFN RMSE:", rmse)
print("Active MFTabPFN MAE:", mae)
print("Active MFTabPFN R2:", r2)
print('N_low:', N_low)
print('N_high:', N_high)
