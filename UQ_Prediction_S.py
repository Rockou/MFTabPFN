...
n_simulations = 10
YY_real = [None for _ in range(n_simulations)]
YY_list = [[None for _ in range(3)] for _ in range(n_simulations)]
for j in range(0, n_simulations):
    ...
    YY_real[j] = YY_total_yuan
    YY_list[j][0] = Y_test_prediction_initial
    # with open('YY_real.pkl', 'wb') as f:
    #     pickle.dump(YY_real, f)
    # with open('YY_list.pkl', 'wb') as f:
    #     pickle.dump(YY_list, f)
    ##########################################################################AutoGluon
    ...
    mae = 1 - mean_absolute_error(YY_total_yuan, Y_test_prediction_initial) / (np.max(YY_total_yuan) - np.min(YY_total_yuan))
    r2 = r2_score(YY_total_yuan, Y_test_prediction_initial)
    YY_list[j][1] = Y_test_prediction_initial
    # with open('YY_list.pkl', 'wb') as f:
    #     pickle.dump(YY_list, f)
    ##########################################################################MFTabPFN
