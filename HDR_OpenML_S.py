...
y_list = []
name_list = []
metadata_path = os.path.join(SAVE_DIR, 'final_datasets.csv')
final_filtered_datasets = pd.read_csv(metadata_path)
print(final_filtered_datasets[['did', 'name', 'NumberOfInstances', 'NumberOfFeatures']])
for did in final_filtered_datasets['did']:
    dataset_path = os.path.join(SAVE_DIR, f'dataset_{did}.pkl')
...
Results_list = [None for _ in range(len(y_list))]
for j in range(0, len(y_list)):
    print(j)
    XX_yuan = X_list[j].values
    YY_yuan = y_list[j].values
    XX_train_yuan, XX_total_yuan, YY_train_yuan, YY_total_yuan = train_test_split(
        XX_yuan, YY_yuan,
        test_size=0.1,
        train_size=0.9,
        random_state=42,
    ...
    nx = XX_yuan.shape[1]
    N_train = XX_train_yuan.shape[0]
    N = XX_total_yuan.shape[0]
    ...
    Results_list[j] = pd.DataFrame(results_list)
    # with open('Results_list.pkl', 'wb') as f:
    #     pickle.dump(Results_list, f)
    # print("Results_list is saved to 'Results_list.pkl'")
