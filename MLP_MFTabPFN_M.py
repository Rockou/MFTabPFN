...
def MLP_M(task_type, INPUT, OUTPUT, input_TabPFN, bili, n_layer, lr1, weight_decay1, Epoch1, hidden_channels1, activate, batch_size1, N_low, N_high):
    ...
    model = MLP(input_dimension=input_dimension, hidden_channels=hidden_channels, output_dimension=output_dimension, n_hidden_layer=n_layer - 1, activate=activate)  # 64
    # print(model)
    ...
    reg.fit(train_input_low.detach().numpy(), train_output_low.detach().numpy().ravel())

    TabPFN_prediction_initial = torch.tensor(reg.predict(train_input_high.detach().numpy()).reshape(-1, 1),dtype=torch.float,requires_grad=False)#False
    ...
    def train():
        ...
        for batch_input, batch_output_delt, TabPFN_prediction_low_high in train_loader:
            out = model(batch_input)
            TabPFN_prediction_tensor, Lower, Upper, Variance = TabPFN_model_main(
                path_to_base_model="auto",
                save_path_to_fine_tuned_model=save_path_to_fine_tuned_model,
                X_train=out,
                y_train=batch_output_delt - model.afa * TabPFN_prediction_low_high,
                X_test=out,
                n_classes=None,
            ...
            loss = criterion(TabPFN_prediction_tensor.to('cpu'), batch_output_delt - model.afa * TabPFN_prediction_low_high)
            optimizer.zero_grad()
            loss.backward()
